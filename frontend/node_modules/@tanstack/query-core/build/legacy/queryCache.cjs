...
var QueryCache = class extends require_subscribable.Subscribable {
	...
	*/
	remove(query) {
		if (require_classPrivateFieldSet2._classPrivateFieldGet2(_queries, this).get(query.queryHash) === query) {
			query.destroy();
			require_classPrivateFieldSet2._classPrivateFieldGet2(_queries, this).delete(query.queryHash);
			this.notify({
				type: "removed",
				query
			});
